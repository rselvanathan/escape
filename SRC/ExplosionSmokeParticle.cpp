...
/* Constructor */
ExplosionSmokeParticle::ExplosionSmokeParticle(int ttl, CVector3 position, int MaxParticles) 
...
void ExplosionSmokeParticle::SetupParticles()
{
	...
	texture = PreLoader::GetInstance()->smoke->texture[0];
	...
	for(ParticleList::iterator it = particles.begin(); it != particles.end(); it++)
	{
		...
		(*it)->mAcceleration.x = 0.3f;	// Acceleration towards certan direction (in this case imitate wind)						
